...
describe Sass::Script::Functions do
  ...
  describe "#escape_css" do
    ...
    it "escapes strings" do
      expect(sass(%q{escape-css('Ich ♥ Bücher')}))
        .to eq(%q{"Ich \\\\2665  B\\\\FC cher"})
    end

    it "preverves identifiers" do
      expect(sass("escape-css(foo)"))
        .to eq("foo")
    end

    it "accepts options" do
      expect(sass(%q{escape-css('lolwat"foo\'bar', $escape-everything: true)}))
        .to eq(%q{"\\\\6C\\\\6F\\\\6C\\\\77\\\\61\\\\74\\\\\\"\\\\66\\\\6F\\\\6F\\\\'\\\\62\\\\61\\\\72"})
    end

    it "accepts falsy options" do
      expect(sass(%q{escape-css('lolwat"foo\'bar', $escape-everything: false)}))
        .to eq(%q{"lolwat\"foo\\\\'bar"})
    end

    it "escape numbers" do
      expect(sass("escape-css(10px, $is-identifier: true)"))
        .to eq("\\31 0px")
    end

    it "properly unquotes" do
      expect(sass("unquote(escape-css('+1', $is-identifier: true))"))
        .to eq("\\+1")
    end
